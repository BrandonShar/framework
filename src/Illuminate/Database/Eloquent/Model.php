...
abstract class Model implements ArrayAccess, Arrayable, Jsonable, JsonSerializable, QueueableEntity, UrlRoutable
{
    ...
    public function delegatesTo($name, ...$delegationChain)
    {
        if (! $delegationChain) {
            // if no delegation chain was provided, then they must've not provided
            // a name and we should get it from the backtrace. What they provided is probably
            // the only delegate that they neeeded. 
            $delegationChain[] = $name;

            list($current, $caller) = debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS, 2);
            $name = $caller['function'];
        }

        //what were they after? Just return whatever we short circuited on?
        if (! $this->{$delegationChain[0]}) {
            //what were they expecting to get back?

            $finalRelation = array_pop($delegationChain);
            $finalDelegator = array_pop($delegationChain);
            $finalDelegator = new $finalDelegator();

            //if it's a relationship, we need to set it.
            if (method_exists($finalDelegator, $finalRelation)) {

                return $this->setRelation($finalRelation, 
                    strpos(get_class($finalDelegator->$finalrelation(), 'Many') !== false) 
                        ? new Collection() : null
                );
            }
        ...
        if (count($delegationChain) === 1) {

            $finalRelation = $this->{$delegationChain[0]};

            return (array_key_exists($name, $finalRelation->attributes) || $finalRelation->hasGetMutator($name)) 
                ? $finalRelation->$name : $finalRelation->$name();
        }

        return $this->{array_shift($delegationChain)}->delegatesTo($name, ...$delegationChain);
